(function(global) {
   ...
   * @param {object} obj1 - The object to extend.
   * @param {object} obj2 - The overrides to apply onto obj1.
   * @return {object} the resulting obj1
   */
  function _extendRecursive(obj1, obj2) {
    ...
    for (const i in obj2) {
      if (obj2.hasOwnProperty(i)) {
        ...
        else obj1[i] = _extendRecursive(obj1[i], obj2[i]);
      }
    }
    return obj1;
  }
  ...
  function Pig(imageData, options) {
    ...
    this.settings = {
      ...
      eventHandlers: {
        ...
        mouseenter: function(elem, info) { elem.classList.add('hover') },
        mouseleave: function(elem, info) { elem.classList.remove('hover') }
      },

      /**
       * Get the list of buttons to appear on hovering over an image.
       *
    ...
    this.settings = _extendRecursive(this.settings, options || {});
  ...
  Pig.prototype._parseImageData = function(imageData) {
    ...
    imageData.forEach(function(image, index) {
      ...
      progressiveImages['key'+image.id] = progressiveImage;
    }.bind(this));
  ...
  Pig.prototype._computeLayout = function() {
    ...
    for (let img in this.images) {
      ...
      // images.
      if (rowAspectRatio >= this.minAspectRatio || index + 1 === Object.keys(this.images).length) {
  ...
  ProgressiveImage.prototype.getElement = function() {
    if (!this.element) {
      ...
      if (Object.keys(this.pig.settings.eventHandlers).length > 0) {
        for (let event in this.pig.settings.eventHandlers)
        {
      ...
      if (this.pig.settings.actionButtons.length > 0) {
        let buttonNum = 1;
        for (let b in this.pig.settings.actionButtons)
        {
          let button = this.pig.settings.actionButtons[b];
